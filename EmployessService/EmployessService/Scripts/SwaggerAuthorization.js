    ...
    $(function () {
        ...
        $('#sa-btn-login').click(function () {
            ...
            */
            ...
            var credentials = $('#sa-username').val() + ":" + $('#sa-password').val();
            credentials = btoa(credentials);
            swaggerUi.api.clientAuthorizations.add("key", new SwaggerClient.ApiKeyAuthorization("Authorization", "Basic " + credentials, "header"));
            $('#sa-btn-setting').text('Hi ' + loginData.user).css('background', '#0f6ab4');
            $('#sa-setting').fadeOut();
